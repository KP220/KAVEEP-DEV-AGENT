...
export async function runIterativeEngineeringLoop(request,registry,options={}){
  ...
  try{validateRequest(request);const verified=await verifySecureSandbox(request.manifestRef);if(verified.manifest.sandboxId!==request.sandboxId)throw new Error("Loop sandbox identity mismatch.");let brainRequest=structuredClone(request.brainRequest);let feedback="";let contextPaths=new Set(brainRequest.contextFiles.map(x=>x.path));
    for(let attempt=1;attempt<=request.maxAttempts;attempt++){
      ...
      const brainResult=await runEngineeringBrain(brainRequest,registry,{clock:options.clock,proposalSchema:options.proposalSchema});
      ...
      try{record.editResult=await editSandbox(request.manifestRef,proposal.proposedEdits,{timestamp:(options.clock?.()??new Date()).toISOString()});}catch(error){record.outcome="edit_failed";base.status="failed";base.errors.push(error.message);base.recommendedNextAction="revise_manually";break;}
