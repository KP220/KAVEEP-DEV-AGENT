...
async function collect(directory, predicate, output = []) { for (const item of await readdir(directory, { withFileTypes: true })) { const target = path.join(directory, item.name); if (item.isDirectory()) await collect(target, predicate, output); else if (predicate(target)) output.push(target); } return output; }
async function certificationFromEvidence(root, packageVersion, name) { try { const evidence = JSON.parse(await readFile(path.join(root, "evidence", name), "utf8")); return evidence.schemaVersion === "1.0.0" && evidence.packageVersion === packageVersion && evidence.platform === process.platform && evidence.status === "certified" ? "certified" : "not_certified"; } catch { return "not_certified"; } }
export async function assessReleaseReadiness(repositoryRoot, options = {}) {
  const root = path.resolve(repositoryRoot); const checks = []; const blockers = []; const warnings = []; const pkg = JSON.parse(await readFile(path.join(root, "package.json"), "utf8"));
  const manifestOk = pkg.name === "@kaveep/dev-agent" && /^0\./.test(pkg.version) && pkg.bin?.kaveep === "tools/kaveep.mjs" && pkg.engines?.node === ">=22" && Array.isArray(pkg.files); checks.push({ id: "package_manifest", status: manifestOk ? "passed" : "failed", detail: `${pkg.name}@${pkg.version}` }); if (!manifestOk) blockers.push("package_manifest_invalid");
  ...
  const dpapi = options.dpapiCertificationStatus ?? await certificationFromEvidence(root, pkg.version, "windows-dpapi-current-user.json"); checks.push({ id: "windows_dpapi_certification", status: dpapi === "certified" ? "passed" : "blocked", detail: dpapi }); if (dpapi !== "certified") blockers.push("windows_dpapi_not_certified");
  blockers.push("interactive_streaming_ux_pending", "multi_language_live_certification_pending");
  warnings.push("The CLI is command-oriented and does not yet stream an interactive coding session.", "Live isolation certification currently proves the Node profile probe only.");
  return { releaseReadinessId: `release_readiness_${pkg.version.replace(/[^0-9A-Za-z]+/g, "_")}`, version: pkg.version, status: blockers.length ? "preview_ready_with_blockers" : "production_ready", installablePreview: !blockers.some((item) => ["package_manifest_invalid", "module_syntax_failed", "package_dry_run_failed"].includes(item)), productionReady: blockers.length === 0, checks, blockers: [...new Set(blockers)], warnings, assessedAt: (options.clock?.() ?? new Date()).toISOString() };
}
