...
async function inspectRepositoryPath(repoRoot, repositoryPath) {
  try {
    const absolutePath = resolveInsideRepository(repoRoot, repositoryPath);
    const stat = await lstat(absolutePath);

    if (stat.isSymbolicLink()) {
      return { status: "MISMATCHED", reason: "symbolic_link" };
    }

    if (!stat.isFile()) {
      return { status: "MISMATCHED", reason: "not_a_file" };
    }
...
async function readJsonFile(repoRoot, repositoryPath) {
  const absolutePath = resolveInsideRepository(repoRoot, repositoryPath);
