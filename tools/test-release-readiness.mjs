...
assert.equal(report.status, "preview_ready_with_blockers"); assert.equal(report.installablePreview, true); assert.equal(report.productionReady, false); assert(report.blockers.includes("live_container_not_certified")); assert.equal(report.blockers.includes("dynamic_tool_using_engineering_loop_pending"), false); assert.equal(report.blockers.includes("interactive_streaming_ux_pending"), false); assert(report.blockers.includes("multi_language_live_certification_pending")); assert.equal(report.blockers.includes("windows_dpapi_not_certified"), false); assert.equal(report.blockers.includes("concurrent_session_scaling_pending"), false); assert.equal(report.blockers.includes("package_dry_run_failed"), false); assert.equal(report.checks.find((item) => item.id === "module_syntax").status, "passed");
